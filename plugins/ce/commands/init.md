...
**Before generating or auditing any files**, load `Skill(ce:authoring-instructions)` for best practices on writing rules, CLAUDE.md, and skills. Load its `references/rules-and-memory.md` reference for rules and CLAUDE.md specifics, and `references/skills.md` if generating project-specific skills.
...
All generated files (rules, CLAUDE.md, skills) must follow the principles from `ce:authoring-instructions`. Key points:
...
Use `ce:authoring-instructions` principles (and its references) as the baseline for what "good" looks like. Check for:
...
**Progressive disclosure violations (per ce:authoring-instructions):**
